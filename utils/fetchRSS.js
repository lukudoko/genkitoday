...
import { setHours, getHours, setMinutes, setSeconds, isAfter, isBefore, parseISO, startOfDay, addDays } from 'date-fns';
import analyseSentiment from './analyseSentiment'; // Import the sentiment analysis function
import cache from './cache';
import shuffleArray from './shuffle';

const parser = new Parser();
const CACHE_KEY = 'rssFeed';
const CACHE_DURATION = 6 * 60 * 60 * 1000; // 8 hours in milliseconds
...
// Function to get the start and end times of the previous chunk
function getPreviousChunkTimes() {

  const now = new Date();
  //const now = new Date('2024-07-22T07:00:00'); 
...
async function fetchRSS() {
  ...
  const cachedNews = cache.get(CACHE_KEY);
  if (cachedNews) {
    return cachedNews;
  }
  ...
  // Shuffle the items once before caching
  const shuffledItems = shuffleArray(allItems);

  // Cache the fetched items
  cache.set(CACHE_KEY, shuffledItems, CACHE_DURATION);
